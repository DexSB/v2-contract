...
import { FTGameHandicapHandler } from "../../src/handler/FTGameHandicapHandler.sol";
import { TicketStatus, ResultStatus } from "../../src/constant/Status.sol";

contract FTGameHandicapHandlerTest is Test {
    FTGameHandicapHandler private handler;

    address private oracle = makeAddr("oracle");

    function setUp() public {
        handler = new FTGameHandicapHandler();
    }

    function test__settleTicket_with_static_case() public {
        ...
        _given_OracleV1_getScore(0, 0);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 50, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.Lost));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 50, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.Won));
        assertEq(deadHeat, 1);

        _given_OracleV1_getScore(1, 0);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 125, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfLost));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 125, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfWon));
        assertEq(deadHeat, 1);

        _given_OracleV1_getScore(0, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 0, 125));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfWon));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 0, 125));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfLost));
        assertEq(deadHeat, 1);
    }

    function test__settleTicket_with_refund() public {
        TicketStatus ticketStatus;
        uint256 deadHeat;
