...
import { FTOverUnderHandler } from "../../src/handler/FTOverUnderHandler.sol";
import { TicketStatus, ResultStatus } from "../../src/constant/Status.sol";

contract FTOddEvenHandlerTest is Test {
    FTOverUnderHandler private handler;

    address private oracle = makeAddr("oracle");

    function setUp() public {
        handler = new FTOverUnderHandler();
    }
    ...
    function test_settleTicket_with_static_case() public {
        ...
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 100, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.Draw));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 100, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.Draw));
        assertEq(deadHeat, 1);

        _given_OracleV1_getScore(1, 0);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 150, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.Lost));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 150, 0));
