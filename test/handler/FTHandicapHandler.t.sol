...
contract FTHandicapHandlerTest is Test {
    FTHandicapHandler private handler;

    address private oracle = makeAddr("oracle");

    function setUp() public {
        handler = new FTHandicapHandler();
    }
    ...
    function test_settleTicket_half_won_lost_with_static_case() public {
        ...
        _given_OracleV1_getScore(1, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("h", 25, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfLost));
        assertEq(deadHeat, 1);
        (ticketStatus, deadHeat) = handler.settleTicket(address(oracle), _given_SingleBetTicketBytes("a", 25, 0));
        assertEq(uint256(ticketStatus), uint256(TicketStatus.HalfWon));
        assertEq(deadHeat, 1);
    }
