...
contract CorrectMapScoreHandler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.CorrectMapScore, ticket.resource1);
        bool isRefund = RefundChecker.getIsRefund(BetType.CorrectMapScore, resultStatus, ticket.resource1);
        if (isRefund) {
