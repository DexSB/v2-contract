...
contract FT1x2Handler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.FT1x2, 0);
        bool isRefund = RefundChecker.getIsRefund(BetType.FT1x2, resultStatus, ticket.resource1);
        if (isRefund) {
