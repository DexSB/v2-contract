...
contract HTOverUnderHandler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.HTOverUnder, ticket.resource1);
        bool isRefund = RefundChecker.getIsRefund(BetType.HTOverUnder, resultStatus, ticket.resource1);
        if (isRefund) {
            return (TicketStatus.Refund, ticket.resource1);
        }
