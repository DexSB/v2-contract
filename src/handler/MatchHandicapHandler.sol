...
contract MatchHandicapHandler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.MatchHandicap, ticket.resource1);
        bool isRefund = RefundChecker.getIsRefund(BetType.MatchHandicap, resultStatus, ticket.resource1);
        if (isRefund) {
        ...
        (uint256 homeScore, uint256 awayScore) = IOracleV1(oracle).getScore(
            ticket.eventId,
            ticket.sportType,
            BetType.MatchHandicap,
            ticket.resource1
        );

        if (ticket.sportType == SportType.Soccer) {
            homeScore -= ticket.liveHomeScore;
            awayScore -= ticket.liveAwayScore;
        }

        homeScore *= 100;
        awayScore *= 100;

        int256 ticketResult = (int256(homeScore) - int256(ticket.hdp1)) - (int256(awayScore) - int256(ticket.hdp2));
