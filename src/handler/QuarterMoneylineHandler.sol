...
contract QuarterMoneylineHandler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.QuarterMoneyline, ticket.resource1);
        bool isRefund = RefundChecker.getIsRefund(BetType.QuarterMoneyline, resultStatus, ticket.resource1);
        if (isRefund) {
