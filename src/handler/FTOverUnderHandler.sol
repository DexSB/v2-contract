...
contract FTOverUnderHandler is IBetTypeHandler {
    function settleTicket(address oracle, bytes memory ticketBytes) external view returns (TicketStatus status, uint256 deadHeat) {
        ...
        ResultStatus resultStatus = IOracleV1(oracle).getEventStatus(ticket.eventId, BetType.FTOverUnder, 0);
        bool isRefund = RefundChecker.getIsRefund(BetType.FTOverUnder, resultStatus, ticket.resource1);
        if (isRefund) {
